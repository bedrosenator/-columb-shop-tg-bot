...
export class BotUpdate {
    ...
    async onExport(@Ctx() ctx: Context) {
        ...
        try {
            ...
            await ctx.replyWithDocument(
                { source: buffer, filename: `expenses_report_${new Date().toISOString().split('T')[0]}.xlsx` },
    ...
    async onMessage(@Ctx() ctx: Context & { message: { text: string } }) {
        ...
        // Игнорируем сообщения, которые являются командами (начинаются со слеша)
        ...
        if (!shopMatch || !cashboxMatch || !terminalMatch || !morningCashMatch || !expensesMatch || !salaryMatch) {
            await ctx.reply(
        ...
        try {
            ...
            if (groupId) {
                const forwardText =
                    ...
                    `💵 **Касса (наличные)**: ${cashbox.toLocaleString('ru-RU')} руб.\n` +
                    `💳 **Терминал (безнал)**: ${terminalTurnover.toLocaleString('ru-RU')} руб.\n` +
                    `🌅 **Утренний баланс**: ${morningCash.toLocaleString('ru-RU')} руб.\n` +
                    `📉 **Расходы**: ${expenses.toLocaleString('ru-RU')} руб.\n` +
                    `💰 **Зарплата**: ${salary.toLocaleString('ru-RU')} руб.\n\n` +
                    `📅 **Дата**: ${new Date().toLocaleDateString('ru-RU')}`;

                await ctx.telegram.sendMessage(groupId, forwardText, { parse_mode: 'Markdown' });
            }

            await ctx.reply('✅ Отчет успешно принят, сохранен в базу и переслан руководству!');
        } catch (error) {
            console.error('Ошибка обработки отчета:', error);
            await ctx.reply('❌ Произошла ошибка при сохранении отчета. Попробуйте еще раз позже.');
        }
    }
}
